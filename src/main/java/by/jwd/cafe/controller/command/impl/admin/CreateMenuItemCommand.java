...
public class CreateMenuItemCommand implements Command {
    ...
    public Router execute(HttpServletRequest request) throws CommandException {
        ...
        try {
            ...
            boolean result = service.addMenuItem(menuItemData);
            int sizeAfter = menuItemData.size();
            if (sizeBefore == sizeAfter) {
                session.removeAttribute(MENU_ITEM_DATA_SESSION);
                session.setAttribute(CREATE_MENU_ITEM_RESULT, result);
            } else {
                session.setAttribute(MENU_ITEM_DATA_SESSION, menuItemData);
            }
            session.setAttribute(CURRENT_PAGE, PagePath.EDIT_MENU);
            router = new Router(PagePath.CREATE_MENU_ITEM, Router.Type.REDIRECT);

        } catch (ServiceException e) {
            logger.error("Try to execute CreateMenuItemCommand was failed.", e);
            throw new CommandException("Try to execute CreateMenuItemCommand was failed.", e);
        }
        return router;
    }
    ...
    private void updateMenuItemData(HttpServletRequest request, Map<String, String> menuItemData) {
        ...
        menuItemData.put(MENU_ITEM_TYPE_SESSION, request.getParameter(RequestParameter.MENU_ITEM_TYPE));
